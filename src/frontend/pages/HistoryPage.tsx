...
import { FaSearch, FaStar, FaShare, FaHeart, FaArrowLeft } from 'react-icons/fa';
...
function HistoryPage() {
  ...
  const filteredResponses = responses.filter(response => {
    const matchesSearch = response.summary.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         response.tool_name.toLowerCase().includes(searchTerm.toLowerCase());
    
    if (filter === 'favorites') return matchesSearch && response.is_favorite;
    if (filter === 'shared') return matchesSearch && (response.is_shared_public || response.is_shared_team);
    ...
    <AppLayout>
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Your saved AI tool responses and favorites</p>
        </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search by summary or tool name..."
                  value={searchTerm}
            ...
            <div className="flex gap-2">
              ...
              >
                Favorites ({responses.filter(r => r.is_favorite).length})
              </button>
              <button
              ...
              >
                Shared ({responses.filter(r => r.is_shared_public || r.is_shared_team).length})
              </button>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 mt-2">Loading your saved responses...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {searchTerm || filter !== 'all' ? 'No matching responses' : 'No saved responses yet'}
            </h3>
            <p className="text-gray-600">
              {searchTerm || filter !== 'all' 
                ? 'Try adjusting your search or filters'
                : 'Start using AI tools and save your responses to see them here'
              }
            </p>
            {!searchTerm && filter === 'all' && (
              <Link
                ...
                <div key={response.id} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      {(response.is_shared_public || response.is_shared_team) && (
                        <div className="p-2 text-green-600" title="Shared">
