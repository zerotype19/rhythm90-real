// Script to update all tools with team context placeholders
...
async function updateAllToolsWithTeamContext() {
  try {
    ...
    console.log('Found prompts:', prompts.map(p => p.tool_name));

    // Update each tool
    for (const [toolName, updateData] of Object.entries(toolUpdates)) {
      const prompt = prompts.find((p: any) => p.tool_name === toolName);
...
3. Run: updateAllToolsWithTeamContext()
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { updateAllToolsWithTeamContext, toolUpdates };
} 